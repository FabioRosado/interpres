...
export function markdownToSafeHtml(markdown: string): string {
  ...
  for (const line of lines) {
    ...
    if (footnote) {
      flushParagraph();
      footnotes.push(`<li><b>${escapeHtml(footnote[1])}.</b> ${inlineMarkdown(footnote[2])}</li>`);
      continue;
    }
...
export function wrapMarkdownSelection(textarea: HTMLTextAreaElement, action: string): void {
