...
# new pipeline.
PIPELINE_VERSION = "v4"
QWEN_DRAFT_FILE = Path(r"C:\Users\FabioRosado\book1-qwen35-v4.jsonl")
MISTRAL_DRAFT_FILE = Path(r"C:\Users\FabioRosado\book1-mistral-v4.jsonl")
REVIEW_FILE = Path(r"C:\Users\FabioRosado\book1-reviewed-v4.jsonl")

QWEN_DRAFT_MODEL = "qwen3.5:9b"
MISTRAL_DRAFT_MODEL = "mistral-small3.2:24b"
QWEN_REVIEW_MODEL = "qwen3.8:27b"

OLLAMA_URL = "http://127.0.0.1:11434/api/chat"

BOOK_NUMBER = 1

# Sentence-aware chunks. The review prompt contains Latin + two complete
# witnesses + source annotations, so keep chunks conservative.
TARGET_CHARS = 2200
MAX_CHARS = 2800
...
def make_chunks(parsed: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def flush():
        ...
        start_offset = current[0][0]
        end_offset = current[-1][1]
    ...
    for sentence in sentences:
        ...
        prospective = current_chars + 1 + sentence_len
...
def run_review_pass(
    ...
    *,
    ...
    for chunk in selected:
        ...
        if qwen is None or mistral is None:
    ...
    if missing:
        raise RuntimeError(
            "Some chunks do not yet have both witnesses. "
            f"First missing IDs: {missing[:5]}."
        )

    print("\n" + "=" * 72)
    print("PASS 2 — QWEN 3.8 ADJUDICATION")
    print("=" * 72)
    ...
    for index, chunk in enumerate(selected, start=1):
        ...
        try:
            ...
            try:
                ...
                debug_file = Path(
                    rf"C:\Users\FabioRosado\review-debug-v4-{chunk_id}.txt"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Commentary on Ezekiel Book I — pipeline v4."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "review = Qwen 3.8 adjudication; "
            "all = both witnesses then adjudication"
        ),
    )
